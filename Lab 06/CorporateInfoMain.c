...
1. Data members which are related are grouped into a data structure. What about functions that act on that data?
   Would it be convenient to somehow group functions that act on a data structure with that data structure somehow?
   Give reasons why or why not.

   Ans: We use classes in C++ in order to do that. Unfortunately C is not object oriented language but it has
   structures to make our work a little bit easier.

2. If you pass an array to a function by address, is it possible for the function to know the length of the array?
   If not, how could we tell the function the length of the array?

   Ans: There is no way to determine the length inside the function. However you pass array, sizeof(arr) will
   always return the pointer size.

3. Databases are based on a schema. This is a schematic of how data is stored. What would be the schema for employee
   information?

   Ans: There are two main kinds of database schema: A logical database schema conveys the logical constraints that
   apply to the stored data. It may define integrity constraints, views, and tables. A physical database schema lays
   out how data is stored physically on a storage system in terms of files and indices.

*/
#include "CorporateInfo.c"
	...
	if (retVal == 0)
	{
		...
		{ // Complete these for lab 6
			...
			case 1:
				...
				if (valid)
					PrintEmployeeInfo(&corporation, index);
				break;
			case 2:
			...
			case 3:
				valid = FindEmployeeByName(&corporation, &index);
				if (valid)
					CorrectEmployeeName(&corporation, index);

				break;
			case 4:
				valid = FindEmployeeByName(&corporation, &index);
				if (valid)
					CorrectEmployeeAge(&corporation, index);
				break;
			case 5:
				valid = FindEmployeeByName(&corporation, &index);
				if (valid)
					ChangeEmployeePosition(&corporation, index);
				break;
			case 6:
				valid = FindEmployeeByName(&corporation, &index);
				if (valid)
					ChangeEmployeeSalary(&corporation, index);
				break;
			case 7:
